  ...
  <div class="spec">
    ...
    <div class="core">
      ...
      <Swiper :slides-per-view="swiperOptions.slidesPerView" :space-between="swiperOptions.spaceBetween"
        @mouseenter="swiperPause" @mouseleave="swiperPlay" class="nice-good-swiper swiper-no-swiping">
        <SwiperSlide v-for="slide of spec.niceGood">
          <a :href="slide.href" target="_blank">
            <img class="icon" :src="slide.src" alt="">
            <p class="name">{{slide.title}}</p>
          </a>
        </SwiperSlide>
        <SwiperSlide v-for="slide of spec.niceGood?.slice(0, 5)">
          <a :href="slide.href" target="_blank">
            <img class="icon" :src="slide.src" alt="">
            <p class="name">{{slide.title}}</p>
          </a>
    ...
    <div class="core">
      <div class="new-arrival">
        ...
        <Swiper :modules="newArrivalSwiperModules" :slides-per-view="newArrivalSwiperOptions.slidesPerView"
          :centered-slides="newArrivalSwiperOptions.centeredSlides" :autoplay="newArrivalSwiperOptions.autoplay"
          :loop="newArrivalSwiperOptions.loop" :looped-slides="newArrivalSwiperOptions.loopedSlides"
          :navigation="newArrivalSwiperOptions.navigation" :speed="newArrivalSwiperOptions.speed"
      ...
      <div class="joy">
        ...
        <div class="tab-hd">
          <a v-for="(hd, index) of spec.joy" :href="hd.head.href" class="item" :class="{active: joyIndex == index}"
            @mouseenter="joyIndex = index">{{hd.head.title}}</a>
        </div>
        <div v-for="(bd, index) of spec.joy" class="tab-body" :class="{active: joyIndex == index}">
          <a v-for="item of bd.body" :href="item.href" target="_blank" class="item">
            ...
            <div class="txt">
              ...
              <p class="price">
                <span class="dollar">¥</span>
                <span class="num">{{Math.trunc(item.price)}}.<span class="decimal">{{(item.price *
...
let spec = ref({}), nowTime, lastTime, countdown = ref({ d: 0, h: 0, m: 0, s: 0 }),
  countdownTime = computed(() => {
    if (countdown.value.d > 1) {
      return countdown.value.d + '天'
    } else {
      return (countdown.value.h < 10 ? '0' + countdown.value.h : countdown.value.h) +
        ":" + (countdown.value.m < 10 ? '0' + countdown.value.m : countdown.value.m) +
        ":" + (countdown.value.s < 10 ? '0' + countdown.value.s : countdown.value.s)
    }
  })

function lingthningBuyCountDown() {
  nowTime = new Date().getTime()
  lastTime = new Date(spec.value.lightningBuy[0]?.extra).setHours(0)
  if (lastTime - nowTime < 1) {
    //重新请求品牌闪购,重置lightningBuy数据
  } else if (Math.floor((lastTime - nowTime) / 1000) / 86400 > 1) {
    countdown.value.d = Math.trunc(Math.floor((lastTime - nowTime) / 1000) / 86400)
  } else {
    countdown.value.h = Math.floor(Math.floor((lastTime - nowTime) / 1000) / 3600)
    countdown.value.d = Math.floor(Math.floor((lastTime - nowTime) / 1000) / 60 % 60)
    countdown.value.s = Math.floor(Math.floor((lastTime - nowTime) / 1000) % 60)
    setInterval(() => {
      if (countdown.value.s > 0) countdown.value.s--
      else if (countdown.value.m > 0) {
        countdown.value.m--
        countdown.value.s = 59
      } else if (countdown.value.h > 0) {
        countdown.value.h--
